...
int fd; 
#define serial_buf	1024

char recv_buf[serial_buf];
...
void *pthread_serial(void *arg)
{
	...
	while(1)
	{
		...
		bytes_read = UART0_Recv(fd, recv_buf,16);

		if(bytes_read > 0)
		{
			printf("receive data is %s len =\n",recv_buf,bytes_read);
		}
		else
		{
	...
	pthread_exit("Thank you for the cpu time in serial-pork task");
...
int main()
{
	...
	pthread_t tid;
	if(pthread_create(&tid,NULL,pthread_serial,NULL)<0)
 	{  
	...
	{
		delay(0x8000000);
		printf("Hello,I'm main task,serial-port0 is running!\n");
		delay(0x8000000);

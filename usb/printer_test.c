...
#define my_file             "/opt/usb/data_file"
...
write_printer_data()
{
	...
	{	
		...
		{
			retval = write(fd[0].fd, plen, 4); //write file len to the host.
	...
	while (1) 
	{
		...
		if(pnum < 1)
		{
			while (1) 
			{					
				bytes_read = fread(buf, filelen, 1, fd_read);

				if (!bytes_read) 
				{
					break;
				}
				retval = write(fd[0].fd, buf, filelen);
				if (retval < 0) 
				{
					printf("Error %d writing to %s\n",
							fd[0].fd,
		...
		else
		{
			...
			while(1)
			{
				int bytes_read = fread(buf, BUF_SIZE, pnum, fd_read);
				if (!bytes_read) 
				{
					break;
				}
								
				retval = write(fd[0].fd, buf, BUF_SIZE);
				if (retval < 0) 
				{
					printf("Error %d writing to %s\n",
							fd[0].fd,
				...
				if(filelen < 512)
				{
					...
					int bytes_read = fread(buf, filelen, 1, fd_read);
...
read_NB_printer_data()
{
	...
	while (1) 
	{
		...
		bytes_write = fwrite(buf,1, BUF_SIZE,fd_write);
		if(bytes_write != 1){
			printf("write error!\n");
			break;
		}			
